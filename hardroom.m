function [earningsum,dearn,wins]= hardroom(dif)
...
for j=1:runs
    ...
    % sets all the players of the room according to their skill level
    player1 = puzzler_p(j);
    player2 = puzzler_p(j);
    player3 = puzzler_p(j);
    player4 = con_p(j);
    ...
    if skillsum >= dif
        ...
        if skillsum>=(dif+15)
            ...
            dmoney=dmoney+10+10
            %logic for if the connisour player doesn't pay at all 
            if skillsum>=(dif+20)
                moneysum=moneysum;
                dmoney=dmoney;
            else 
                ...
                %more
            moneysum = moneysum+wmoney4+wmoney5+wmoney6+wmoney7+wmoney8;
            dmoney= dmoney+wmoney4+wmoney5+wmoney6+wmoney7+wmoney8;
            end
        else
